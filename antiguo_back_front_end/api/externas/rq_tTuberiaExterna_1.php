...
switch ($request_method) {
    case 'GET':
        ...
        if (!empty($_GET["tubo"])) {
            $T_ID_tubo = addslashes($_GET["tubo"]);
            get_tubo($T_ID_tubo);
        }elseif(!empty($_GET["No_tubo"])) {
            $T_No_tubo= addslashes($_GET["No_tubo"]);  
            get_numero_tubo($T_No_tubo);
...
function get_ID_tubo($T_ID_tubo="")
{
...
function get_numero_tubo($T_No_tubo)
{
    $sql = "SELECT * FROM tuberia_soldadura_externa_1 WHERE Tex_No_tubo=\"" . $T_No_tubo . "\"";
    global $conn;
    $query = $conn->query($sql);
    $data = array();
    while ($r = $query->fetch_assoc()) {
...
{
    @$id_tubo =addslashes($_POST['T_id_tubo']);
    @$T_no_tubo =addslashes($_POST['T_no_tubo']);
    @$T_no_placa = addslashes($_POST['T_no_placa']);
    ...
    if ($conn->query($sql) === TRUE) {
        echo "El tubo ".$id_tubo." fue registrado de externa 1";
    } else {
...
function update_tubo()
{
    ...
    @$id_tubo =addslashes($_POST['A_ID_tubo']);
...
{
    //actualizar la columna a de  la tabla
    @$id_tubo =addslashes($_POST['A_id_tubo']);
...
function update_tuberia(){
    ...
    @$id_tubo =addslashes($_POST['A_data_tubo']);
    @$T_no_tubo =addslashes($_POST['T_no_tubo']);
    @$T_no_placa = addslashes($_POST['T_no_placa']);
    @$T_ID_proyecto = addslashes($_POST['T_ID_proyecto']);
    @$T_lote_alambre = addslashes($_POST['T_lote_alambre']);
...
{
    //actualizar la columna archivos_excel de  la tabla
    @$id_tubo =addslashes($_POST['H_ID_tubo']);
...
//para cargar datos en id de interna 1
function cargar_ID_Rtubo()
{
    @$id_tubo =addslashes($_POST['CA_ID_tubo']);
    @$T_id_Rtubo=addslashes($_POST['CA_ID_Rtubo']);
    global $conn;
    $sql="UPDATE tuberia_soldadura_externa_1 SET Tex_ID_Rtubo='".$T_id_Rtubo."' WHERE Tex_ID_tubo=\"" . $id_tubo . "\"";
    
    if ($conn->query($sql) === TRUE) {
        echo "Se registro/actualizo id Rtubo";
    } else {
        echo "Error: " . $sql . "<br>" . $conn->error;
    }
}
