...
{
    $sql = "SELECT * FROM tuberia_soldadura_externa_3 WHERE Tex_No_tubo=\"" . $T_No_tubo . "\"";
    global $conn;
    $query = $conn->query($sql);
    $data = array();
    while ($r = $query->fetch_assoc()) {
...
function insert_tubo()
{
    ...
    if ($conn->query($sql) === TRUE) {
        echo "El tubo ".$id_tubo." fue registrado de externa 3";
    } else {
...
//para cargar datos en id de externa 3
function cargar_ID_Rtubo()
{
    @$id_tubo =addslashes($_POST['CA_ID_tubo']);
    @$T_id_Rtubo=addslashes($_POST['CA_ID_Rtubo']);
    global $conn;
    $sql="UPDATE tuberia_soldadura_externa_3 SET Tex_ID_Rtubo='".$T_id_Rtubo."' WHERE Tex_ID_tubo=\"" . $id_tubo . "\"";
